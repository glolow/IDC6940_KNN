...
The simpler model seems to have better performance. With a K value of 15, we see an accuracy of 0.86 and a much better Kappa value of 0.74. Both of these scores are higher than we had seen with the larger model and the Kappa of 0.74 indicates that we are predicting both classes better although still not perfectly. Another thing to note is notice how the accuracy "bounces" (drops at k=9, peaks at 11, then drops again at 17). Since we have a smaller sample size (96 versus 312), the model is much more sensitive to how the 10-fold cross-validation splits the data. Another model we could test is combining glucose with another strong predictor (like BMI or Age) to see if we could push that accuracy and Kappa score even higher.[@Google2026]
